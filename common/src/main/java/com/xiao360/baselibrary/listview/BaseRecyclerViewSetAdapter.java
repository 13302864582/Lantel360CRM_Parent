...
public abstract class BaseRecyclerViewSetAdapter<T> extends RecyclerView.Adapter<BaseViewHolder>{
    /**
     *  context 上下文
     */
     ...
     * datas 数据模型
     */
    protected ArraySet<T> datas;

    public ArraySet<T> getDatas() {
        return datas;
    }

    public void setDatas(ArraySet<T> datas) {
        this.datas = datas;
    }
     ...
     * 适配器构造
     */
    public BaseRecyclerViewSetAdapter(Context context, ArraySet<T> datas) {
    ...
    public void onBindViewHolder(BaseViewHolder holder, int position) {
        if(null == datas || position>=datas.size()){
            ...
            bindViewHolder(holder, datas.valueAt(position), position,getItemViewType(position));
        }
    }
